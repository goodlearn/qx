						...
						<li class="borderButtom">
							...
							<select name="carMotorCycleId">
								<option value="0" selected>请选择车型...</option>
									<c:forEach items="${cmcList}" var="cmc">
										<c:if test = "${cmc.id == mm.carMotorCycleId}">
											<option value="${cmc.id}" selected>${cmc.name}</option>
										</c:if>
										<c:if test = "${cmc.id != mm.carMotorCycleId}">
							...
							<select name="carWagonId">
								<option value="0" selected>请选择车号...</option>
									<c:forEach items="${cwList}" var="cw">
										<c:if test = "${cw.id == mm.carWagonId}">
											<option value="${cw.id}" selected>${cw.name}</option>
										</c:if>
										<c:if test = "${cw.id != mm.carWagonId}">
				...
				<div class="taskInfo">
					<ul>
						<li>
							...
							<input name="runTime" value="${mm.runTime}" type="text" placeholder="请输入运行时间...">
						</li>
					</ul>
				</div>
