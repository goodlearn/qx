...
/**
 * MT4400卡车钳工周检分区Service
 * @author wzy
 * @version 2018-02-28
 */
@Service
@Transactional(readOnly = true)
public class ItemMt440ZjfqService extends CrudService<ItemMt440ZjfqDao, ItemMt440Zjfq> {
	...
	public void createMask(ViewMcsi1[] viewMcsi1s,User user) {
		...
		//字典数据检验
		if(!type.equals(DictUtils.getDictValue(Global.ITEM_MT_440, "bussinessType", "1"))) {
			//MT4400卡车钳工周检分区
			return;
		}
		...
		for(ViewMcsi1 viewMsci1 : viewMcsi1s) {
			...
			ItemMt440Zjfq queryMcsi1 = new ItemMt440Zjfq();
			queryMcsi1.setPart(part);
			queryMcsi1.setAssembleId(bussinessAssembleId);
			List<ItemMt440Zjfq> mcsi1List = dao.findList(queryMcsi1);
			...
			for(ItemMt440Zjfq forEntity :mcsi1List) {
				//每个人关联的任务保存
	...
	public ItemMt440Zjfq get(String id) {
		return super.get(id);
	}
	
	public List<ItemMt440Zjfq> findList(ItemMt440Zjfq itemMt440Zjfq) {
		return super.findList(itemMt440Zjfq);
	}
	
	public Page<ItemMt440Zjfq> findPage(Page<ItemMt440Zjfq> page, ItemMt440Zjfq itemMt440Zjfq) {
		return super.findPage(page, itemMt440Zjfq);
	}
	
	@Transactional(readOnly = false)
	public void save(ItemMt440Zjfq itemMt440Zjfq) {
		super.save(itemMt440Zjfq);
	}
	
	@Transactional(readOnly = false)
	public void delete(ItemMt440Zjfq itemMt440Zjfq) {
		super.delete(itemMt440Zjfq);
	}
